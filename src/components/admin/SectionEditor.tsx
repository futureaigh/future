...
export default function SectionEditor({
	...
	const renderField = (
		...
		const isImageUrl =
			...
			key.toLowerCase().includes("ogimage") ||
			...
			return (
				<div key={key} className="space-y-4">
					...
					<div className="space-y-4">
										...
										<div key={field} className="space-y-1.5">
											...
											{isItemImage && fieldValue && (
												<div className="relative group w-16 h-16 mb-1 bg-gray-50 border border-gray-100 rounded-lg flex items-center justify-center p-1">
													...
													<button
														onClick={() =>
															handleArrayChange(key, idx, field, "")
														}
											...
											<input
												...
												onChange={(e) =>
													handleArrayChange(key, idx, field, e.target.value)
												}
											/>
										</div>
									);
						...
						<button
							onClick={() => {
								const newItem = Object.keys(value[0] || {}).reduce(
									(acc: any, k) => ({ ...acc, [k]: "" }),
									{},
								);
					...
					<div className="p-6 bg-white border border-gray-100 rounded-3xl space-y-4 shadow-sm">
						{Object.entries(value).map(([subKey, subValue]) => {
							const isSubImage =
								...
								subKey.toLowerCase().includes("logo") ||
								subKey.toLowerCase().includes("favicon");
							return (
