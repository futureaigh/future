...
export async function saveContent(sectionKey: string, content: any): Promise<void> {
  await j(await fetch(`/api/content/${sectionKey}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function createSubmission(data: any): Promise<void> {
  await j(await fetch('/api/submissions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
